...
desc "Compresses the source with uglifyjs"
task :compress do
    dirname = File.dirname OUTPUT
    filename = File.basename OUTPUT, '.js'
    newname = File.join dirname, "#{filename}.min.js"
    command_echoing_output "uglifyjs -o #{newname} #{OUTPUT}"
end
